...
export function PasswordGen() {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-[var(--bg)] p-4 text-center font-mono text-xl text-emerald-300 break-all">{pwd || "..."}</div>
...
export function PasswordStrength() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="h-3 overflow-hidden rounded-full bg-[var(--bg-recessed)]"><div className={`h-full transition-all ${r.score >= 80 ? "bg-emerald-500" : r.score >= 60 ? "bg-blue-500" : r.score >= 40 ? "bg-amber-500" : "bg-red-500"}`} style={{ width: `${r.score}%` }} /></div>
...
export function QrGen({ wifi = false }: { wifi?: boolean }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-center rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-6">
...
export function BarcodeGen() {
  ...
  return (
    <div className="space-y-3">
      ...
      <canvas ref={ref} width={600} height={160} className="w-full rounded-xl border bg-[var(--bg-elevated)]" />
...
export function Dice() {
  ...
  return (
    <div className="space-y-4 text-center">
      ...
      {hist.length > 0 && <p className="text-xs text-[var(--fg-muted)]">Lịch sử: {hist.map((h, i) => <span key={i} className="mx-1">[{h.join(",")}]</span>)}</p>}
    </div>
  );
}
...
export function Coin() {
  ...
  return (
    <div className="space-y-4 text-center">
      ...
      <p className="text-sm text-[var(--fg-muted)]">Sấp: {stat.S} • Ngửa: {stat.N}</p>
    </div>
  );
}
...
export function RandomNumber() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">{out.map((v) => <span key={v} className="rounded-xl bg-[var(--bg)] px-3 py-1.5 font-mono font-bold text-white">{v}</span>)}</div>
    </div>
  );
}
...
export function RandomName() {
  ...
  return (
    <div className="space-y-3 text-center">
      ...
      <div className="tool-action-area flex justify-center gap-2"><button onClick={gen} className="rounded-xl bg-[var(--bg)] px-4 py-2 text-sm font-semibold text-white">Tên khác</button><CopyBtn text={name} /></div>
    </div>
  );
}
...
export function Wheel() {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <div className="flex flex-col items-center justify-center rounded-2xl border bg-[var(--bg-recessed)] p-6 text-center">
        <p className="text-sm text-[var(--fg-muted)]">Kết quả</p>
